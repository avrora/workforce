...
Template['view'].rendered = function () {
  ...
  GoogleMaps.init({
    ...
    function () {
      ...
      var meteorUser = '';
      if (Meteor.user()) {
        meteorUser = Meteor.user().username;
      }
...
function incrementStep() {
  ...
  if (legs[legsIdx] && legs[legsIdx].steps && legs[legsIdx].steps[stepIdx] && pathIdx == legs[legsIdx].steps[stepIdx].path.length) {
    pathIdx = 0;
  ...
  try {
    ...
    legsIdx = legsIdx || 0;
    if (legs[legsIdx] && legs[legsIdx].steps[stepIdx]) {
      var pathLocation = legs[legsIdx].steps[stepIdx].path[pathIdx];
      if (pathLocation) {
        Meteor.users.update(Meteor.user()._id, {
          $set: {
            'lastLocation': {
              lat: pathLocation.lat(),
              lng: pathLocation.lng()
            }
          }
        });
